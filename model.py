...
class Adagnn_local(Module):
    def __init__(self, layer, in_features):
        super(Adagnn_local, self).__init__()
    ...
    def reset_parameters(self):
        for temp in self.temp_local:
            torch.nn.init.normal_(temp, mean=0.0, std=0.0)  # 先正态采样
            #temp.data = torch.sigmoid(temp.data)  # 用 sigmoid 映射到 (0,1)


    def forward(self, adj, X):
        
        for i in range(self.layer):
            LX = torch.spmm(adj, X) #LX
...
class Adagnn_global(Module):

    def __init__(self, layer, in_features):
        super(Adagnn_global, self).__init__()
        self.in_features = in_features
        self.layer = layer
...
class OC_Auto(nn.Module):
    def __init__(self, in_features, hidden1, hid2, layers, batch_size, tau):
        ...
        self.ada_global = Adagnn_global(layers, hid2)
        self.ada_local = Adagnn_local(layers,hid2)
    ...
    def forward(self, adj, x):
        ...
        x_global = self.ada_global(adj,x_global)
        x_local = self.ada_local(adj,x_local)


        #embedding=torch.cat((x_global,x_local),dim=1)
